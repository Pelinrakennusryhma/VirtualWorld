...
public class ShootingRangeFigureEightMovingTarget : ShootingRangeMovingTarget
{
    ...
    protected override void FixedUpdate()
    {
        ...
        if (moveXZ)
        {
            xzOffset = new Vector3(Mathf.Cos(Time.time * xzMoveSpeed) * xzMoveRadius,
                                   0,
                                   (Mathf.Sin(2 * Time.time * xzMoveSpeed) * xzMoveRadius));
        }
        ...
        if (moveYZ)
        {
            yzOffset = new Vector3(0,
                                   ...
                                   (Mathf.Sin(2 * Time.time * yzMoveSpeed) * yzMoveRadius));
        }

        ActualMovingObject.transform.localPosition = originalLocalPosition + xzOffset + yzOffset;

        //Debug.Log("Figure eight moving target fiexed update is called " + gameObject.name);
